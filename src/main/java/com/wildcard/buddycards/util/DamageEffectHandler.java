...
public class DamageEffectHandler {
    ...
    public void onDamagePlayer (LivingDamageEvent event) {
        if (event.getEntityLiving() instanceof Player entity) {
            if (entity.getItemBySlot(EquipmentSlot.HEAD).getItem().equals(BuddycardsItems.LUMINIS_HELMET.get()) && !entity.getCooldowns().isOnCooldown(BuddycardsItems.LUMINIS_HELMET.get())) {
                if (entity.getHealth() <= event.getAmount()+2) {
                    event.setAmount(1);
                    entity.level.explode(entity, entity.getX(), entity.getY(), entity.getZ(), 2, Explosion.BlockInteraction.NONE);
            ...
            if (event.getEntityLiving().getItemBySlot(EquipmentSlot.FEET).getItem().equals(BuddycardsItems.ZYLEX_BOOTS.get()) && !entity.getCooldowns().isOnCooldown(BuddycardsItems.ZYLEX_BOOTS.get())) {
                if (entity.getHealth() <= event.getAmount()+2) {
                    event.setAmount(1);
                    while(true)
